...
async def create_craftid(data: OnboardingData):
    ...
    try:
        ...
        # do not roll back DB insert; return pending state and log error
        # (batcher can later pick it up if manual intervention)
...
async def verify_craftid(public_id: str):
    ...
    Returns:
    - status: "pending" (queued/not yet anchored), "anchored" (verified on-chain), "tampered" (hash mismatch)
    - Compares stored hash with recomputed hash and on-chain hash
    """
    await ensure_db_ready_or_502()
    ...
    # Check if metadata has been tampered with (stored hash != recomputed hash)
    ...
    if status == "anchored" and tx_hash:
        ...
        try:
            ...
            blockchain_hash_match = anchored  # if anchored, hash matches on-chain
        except Exception as e:
            # Blockchain query failed, but don't fail the request
            blockchain_anchored = False
            blockchain_timestamp = None
